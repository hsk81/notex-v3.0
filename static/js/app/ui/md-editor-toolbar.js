...
define(["require", "exports", "../decorator/named", "../decorator/trace", "./md-editor"], function (require, exports, named_1, trace_1, md_editor_1) {
    ...
    "use strict";
    let MdEditorToolbar = MdEditorToolbar_1 = class MdEditorToolbar {
        constructor() {
            this.$publish
        ...
        static get me() {
            if (this['_me'] === undefined) {
                this['_me'] = window['MD_EDITOR_TOOLBAR'] = new MdEditorToolbar_1();
            }
            return this['_me'];
        }
        refresh() {
        ...
        onHeaderClickMirror() {
            let cursor = this.ed.mirror.getCursor(), curr_from = { line: cursor.line, ch: 0 }, next_from = { line: cursor.line + 1, ch: 0 };
            let curr_ts = this.ed.mirror.getLineTokens(curr_from.line), next_ts = this.ed.mirror.getLineTokens(next_from.line);
            let line = this.ed.mirror.getLineHandle(curr_from.line), mode = this.ed.mirror.getModeAt(curr_from);
            let suffix = line.text.match(/^\s+/) ? '' : ' ', prefix = '#';
        ...
        onHeaderClickSimple() {
            let val = this.ed.$input.val(), beg = this.ed.$input[0].selectionStart, end = this.ed.$input[0].selectionEnd, idx = beg;
            while (idx-- > 0) {
                if (val[idx] === '\n') {
                    break;
                }
            }
            let px = val.substring(0, idx + 1), sx = val.substring(idx + 1, val.length);
            let rx_6 = /^\s*#{6,}\s*/, mm_6 = sx.match(rx_6);
            let rx_5 = /^\s*#{1,5}/, mm_5 = sx.match(rx_5);
            let rx_0 = /^\s*#{0}/, mm_0 = sx.match(rx_0);
        ...
        onBoldClickMirror() {
            ...
            if (selections.length > 0 && this.ed.mirror.getSelection()) {
                this.ed.mirror.setSelections(selections.map((sel) => {
                    let lhs_mod = this.ed.mirror.getModeAt(sel.head), rhs_mod = this.ed.mirror.getModeAt(sel.anchor);
                ...
                this.ed.mirror.replaceSelections(selections.map((sel) => {
                    let sel_lhs = sel.anchor, sel_rhs = sel.head;
                    ...
                    let mod_lhs = this.ed.mirror.getModeAt(sel_lhs), mod_rhs = this.ed.mirror.getModeAt(sel_rhs);
                    if (mod_lhs && mod_lhs.name === 'markdown' &&
                        mod_rhs && mod_rhs.name === 'markdown') {
                        let tok = this.ed.mirror.getTokenAt(sel_rhs);
                        if (tok.type && tok.type.match(/strong/) ||
                            tok.type && tok.type.match(/em/)) {
                            let lhs = this.lhs(sel_lhs, tok), rhs = this.rhs(sel_rhs, tok);
                ...
                let cur = this.ed.mirror.getCursor(), mod = this.ed.mirror.getModeAt(cur);
                if (mod && mod.name === 'markdown') {
                    let tok = this.ed.mirror.getTokenAt(cur);
                    if (tok.type && tok.type.match(/strong/) ||
                        tok.type && tok.type.match(/em/)) {
                        let lhs = this.lhs(cur, tok), rhs = this.rhs(cur, tok);
                        if (tok.type.match(/em/)) {
                            let src = this.ed.mirror.getRange(lhs, rhs), tgt = `*${src}*`;
                            ...
                            let src = this.ed.mirror.getRange(lhs, rhs), tgt = src.slice(2, -2);
                            this.ed.mirror.replaceRange(tgt, lhs, rhs);
                            this.ed.mirror.setSelection(lhs, {
        ...
        onBoldClickSimple() {
            let val = this.ed.$input.val(), beg = this.ed.$input[0].selectionStart, end = this.ed.$input[0].selectionEnd;
            let px_1 = val.substring(0, beg), ix_1 = val.substring(beg, end), sx_1 = val.substring(end, val.length);
            let px_2 = val.substring(0, beg - 2), ix_2 = val.substring(beg - 2, end + 2), sx_2 = val.substring(end + 2, val.length);
            let rx_1 = /^\*\*((?:(?!\*\*).)+)\*\*$/, mm_1 = ix_1.match(rx_1);
            let rx_2 = /^\*\*((?:(?!\*\*).)+)\*\*$/, mm_2 = ix_2.match(rx_2);
            ...
            if (selections.length > 0 && this.ed.mirror.getSelection()) {
                ...
                if (mod && mod.name === 'markdown') {
                    ...
                    if (tok.type && tok.type.match(/strong/) ||
                        tok.type && tok.type.match(/em/)) {
                        let lhs = this.lhs(cur, tok), rhs = this.rhs(cur, tok);
                        let src = this.ed.mirror.getRange(lhs, rhs), tgt = src.slice(1, -1);
                        this.ed.mirror.replaceRange(tgt, lhs, rhs);
                        this.ed.mirror.setSelection(lhs, {
                            line: rhs.line, ch: rhs.ch - 2
                        });
        ...
        onItalicClickSimple() {
            let val = this.ed.$input.val(), beg = this.ed.$input[0].selectionStart, end = this.ed.$input[0].selectionEnd;
            let px_1 = val.substring(0, beg), ix_1 = val.substring(beg, end), sx_1 = val.substring(end, val.length);
            let px_2 = val.substring(0, beg - 1), ix_2 = val.substring(beg - 1, end + 1), sx_2 = val.substring(end + 1, val.length);
            let rx_1 = /^\*([^*]+)\*$/, mm_1 = ix_1.match(rx_1);
            let rx_2 = /^\*([^*]+)\*$/, mm_2 = ix_2.match(rx_2);
            if (mm_1 && mm_1.length > 1) {
                if (!document.execCommand('insertText', false, mm_1[1])) {
        ...
        onIndentClickSimple() {
            let val = this.ed.$input.val(), beg = this.ed.$input[0].selectionStart, end = this.ed.$input[0].selectionEnd, idx = beg - 1;
            while (idx >= 0 && val[idx] !== '\n') {
                idx -= 1;
            }
            let px = val.substring(0, idx + 1), sx = val.substring(idx + 1, val.length);
        ...
        onOutdentClickMirror() {
            this.ed.mirror.execCommand('indentLess');
        }
        onOutdentClickSimple() {
            let val = this.ed.$input.val(), beg = this.ed.$input[0].selectionStart, end = this.ed.$input[0].selectionEnd, idx = beg - 1;
            while (idx >= 0 && val[idx] !== '\n') {
                idx -= 1;
            }
            let px = val.substring(0, idx + 1), sx = val.substring(idx + 1, val.length);
            let rx = /^\s{2}/, mm = sx.match(rx);
            if (mm && mm.length > 0) {
        ...
        onSupscriptClickSimple() {
            let val = this.ed.$input.val(), end = this.ed.$input[0].selectionEnd;
            this.ed.$input[0].setSelectionRange(end, end);
            if (!document.execCommand('insertText', false, '^{ }')) {
                let px = val.substring(0, end), sx = val.substring(end, val.length);
                this.ed.$input.val(`${px}^{ }${sx}`);
            }
        ...
        lhs(cursor, token) {
            if (this.ed.mirror) {
                let last = { line: cursor.line, ch: cursor.ch }, next = { line: cursor.line, ch: cursor.ch };
                while (true) {

...
where the line breaks after the *initial* and before the *trailing* double dollars (<code>&dollar;&dollar;</code>) are  *optional*. The rendered output looks like this:
...
You insert Prezi videos by using a similar syntax as for referencing an image, but again you require an initial at sign (`@`):
